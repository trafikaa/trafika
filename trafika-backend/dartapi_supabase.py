...
def read_delisted_companies(file_path='trafika-backend/src_data/delisted_stocks_2005_2025.csv'):
    """상장폐지 기업 목록을 읽어옵니다"""
...
def main():
    """상장폐지 기업들의 재무제표를 수집합니다 (상장폐지 전년도만)"""
    ...
    for report_name, reprt_code in REPORT_CODES.items():
        ...
        for _, row in delisted_df.iterrows():
            ...
            if target_year < 2005:
                print(f"[SKIP] {ticker}: 상장폐지 전년도({target_year})가 2005년 이전")
                continue
                
            print(f"  → {target_year}년 데이터 수집 (상장폐지 전년도)")
            
            response = fetch_financial_data(corp_code, target_year, reprt_code)
            parsed_rows = parse_full_response(response, ticker, corp_name, delisting_year)
            if parsed_rows:
                save_to_supabase(table_name, parsed_rows)
                print(f"    → {target_year}년 {len(parsed_rows)}건 저장 완료")
            else:
                print(f"    → {target_year}년 데이터 없음")
            time.sleep(0.3)  # API 호출 제한 고려
            
            print(f"[COMPLETE] {ticker} 완료 (상장폐지 전년도 {target_year}년 데이터)")
